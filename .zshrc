...
setup_dev() {
  ...
  if [ $? != 0 ]; then
    # Window 1: Claude Code (top, full width) | two terminals (bottom: left + right)
    tmux new-session -d -s $SESSION -n "Claude Code + Terminals" -c "$dir"
    tmux split-window -v -p 30 -t $SESSION:0.0 -c "$dir"
    tmux split-window -h -p 50 -t $SESSION:0.1 -c "$dir"
